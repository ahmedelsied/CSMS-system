<!DOCTYPE html>
<html lang="ar" dir="rtl">
<head>
    <noscript><meta http-equiv="refresh" content="0; URL=/error_js"></noscript>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>csms <?=$this->notification->count_notif != 0 ? '('.$this->notification->count_notif.')' : ''?></title>
    <link href="<?=IMGS.'logo'.DS?>csms_logo_icon.png" rel="shortcut icon"/>
    <link href="https://fonts.googleapis.com/css2?family=Cairo&display=swap" rel="stylesheet">
    <link href="<?=PUBLIC_CSS?>bootstrap.min.css" rel="stylesheet"/>
    <link href="<?=PUBLIC_CSS?>fontawesome.min.css" rel="stylesheet"/>
    <link href="<?=PUBLIC_CSS?>public_style.css" rel="stylesheet"/>
    <link href="<?=SUPERVISOR_CSS?>supervisor.css" rel="stylesheet"/>
    <script src="<?=PUBLIC_JS?>jquery-3.3.1.min.js"></script>
    <script src="<?=PUBLIC_JS?>functions.js"></script>
</head>
<body>