<?php
namespace controllers\supervisor_controllers;
use lib\vendor\sessionmanger;
use lib\vendor\helper;
use lib\vendor\inputfilter;
use lib\vendor\requests;
use controllers\supervisor_controllers\notification_handle;
use lib\vendor\class_factory;
use controllers\abstract_controller as abstract_controller;
class index_controller extends abstract_controller
{
    use sessionmanger,helper,inputfilter,requests;
    private $governments_model;
    private $shipping_admin_model;
    private $orders_model;

    public $governments;
    public $review_orders;
    public $shipping_admin;
    public function __construct()
    {
        if(!($this->issetSession('user_name') && $this->getSession('user_type') == SUPERVISOR_ID)){
            $this->redirect(DOMAIN.'/login');
        }
        $this->notification = new notification_handle();
        $this->governments_model        = class_factory::createInstance('models\public_models\government');
        $this->shipping_admin_model     = class_factory::createInstance('models\shipping_admin_models\shipping_admin');
        $this->orders_model             = class_factory::createInstance('models\public_models\orders_model');
        $this->governments              = $this->governments_model->getAll();
        
        $seleced = 'shipping_admin.*, inventories.inventory_name';
        $this->shipping_admin = $this->shipping_admin_model->getAllJOIN($seleced,['inventories'],['id'],['inventory_id'],null,\PDO::FETCH_ASSOC);
    }
    public function default_action($params)
    {
        $this->get(function ($params)
        {
            if($this->setParams($params,0)){
                $this->active_link();
                $seleced = 'orders.*, customers_service.full_name as customers_service_full_name';
                $this->review_orders = $this->orders_model->getJOIN($seleced,['customers_service'],['id'],['customers_service_id'],0,'order_status = '.SEND_TO_SUPERVISOR,'LEFT');
                $this->loadHeader(SUPERVISOR_TEMP);
                $this->renderNav(SUPERVISOR_TEMP);
                $this->_view(SUPERVISOR_VIEWS.'await_orders_view');
                $this->loadFooter(SUPERVISOR_TEMP);
            }else{
                $this->invalid_params_action();
            }
        },[$params]);
    }
    public function show_more_action($params)
    {
        $this->ajax(function ($params){
            if($this->setParams($params,0)){
                $offset = $this->filterInt($_POST['offset']);
                $seleced = 'orders.*, customers_service.full_name as customers_service_full_name';
                $this->orders = $this->orders_model->getJOIN($seleced,['customers_service'],['id'],['customers_service_id'],$offset,'order_status = '.SEND_TO_SUPERVISOR);
                foreach($this->orders as $order){
                    $html = '<tr>';
                    $html .=    '<td class="id">'.str_pad($order['id'],4,"0",STR_PAD_LEFT).'</td>';
                    $html .=    '<td class="address">'.$order['address'].'</td>';
                    $html .=    '<td class="gove" data-gove-id="'.$order['government'].'">'.$order['government'].'</td>';
                    $html .=    '<td class="phone">'.$order['phone_number'].'</td>';
                    $html .=    '<td class="full-name">'.$order['full_name'].'</td>';
                    $html .=    '<td class="details">'.$order['order_details'].'</td>';
                    $html .=    '<td class="notes">'.$order['notes'].'</td>';
                    $html .=    '<td class="customers-service" data-service-id="'.$order['customers_service_id'].'">'.$order['customers_service_full_name'].'</td>';
                    $html .=    '<td>';
                        $html .=    '<button class="btn btn-success btn-edit-order">تعديل';
                            $html .=    '<i class="fa fa-edit"></i>';
                        $html .=    '</button>';
                    $html .=    '</td>';
                    $html .=    '<td class="select-order-list"><input type="checkbox" class="order-row" value="'.$order['id'].'"/></td>';

                    $html .='</tr>';
                    echo $html;
                }
            }
        },UPDATE_ORDERS_FROM_SUPERVISOR,[$params]);
    }
    public function update_order_action()
    {
        $this->ajax([$this,'save_orders'],UPDATE_ORDERS_FROM_SUPERVISOR);
    }
    private function save_orders()
    {
        if((isset($_POST['id']) && !empty($_POST['id']))
        && (isset($_POST['address']) && !empty($_POST['address']))
        && (isset($_POST['gove']) && !empty($_POST['gove'])) 
        && (isset($_POST['phone']) && !empty($_POST['phone']))
        && (isset($_POST['full_name']) && !empty($_POST['full_name'])) 
        && (isset($_POST['order_details']) && !empty($_POST['order_details']))
        && (isset($_POST['notes']) && !empty($_POST['notes']))
        ){
            $this->prepareParams($this->orders_model);
            if($this->orders_model->save()){
                echo 'dn';
            }else{
                echo 'err';
            }
        }else{
            echo 'err';
        }
    }
    private function prepareParams($orders_model)
    {
        if(isset($_POST['id'])){
            $orders_model::$id = $this->filterInt($_POST['id']);
        }
        
        $orders_model::$tableSchema['customers_service_id'] = $this->getSession('user_id');
        $orders_model::$tableSchema['address'] = isset($_POST['address']) ? $this->filterString($_POST['address']) : null;
        $orders_model::$tableSchema['government'] = isset($_POST['gove']) ? $this->filterInt($_POST['gove']) : null;
        $orders_model::$tableSchema['phone_number'] = $this->filterString($_POST['phone']);
        $orders_model::$tableSchema['full_name'] = isset($_POST['full_name']) ? $this->filterString($_POST['full_name']) : null;
        $orders_model::$tableSchema['order_details'] = isset($_POST['order_details']) ? $this->filterString($_POST['order_details']) : null;
        $orders_model::$tableSchema['notes'] = isset($_POST['notes']) ? $this->filterString($_POST['notes']) : null;
    }
    public function send_to_shipping_admin_action()
    {
        $this->ajax([$this,'send_to_shipping_admin'],UPDATE_ORDERS_FROM_SUPERVISOR);
    }
    private function send_to_shipping_admin(Type $var = null)
    {
        if((isset($_POST['idList']) && !empty($_POST['idList'])) 
        && (isset($_POST['shipping_admin_id']) && !empty($_POST['shipping_admin_id']))){
            $idList = '';
            foreach($_POST['idList'] as $id){
                if(end($_POST['idList']) == $id){
                    $idList .= !empty($this->filterInt($id)) ? $this->filterInt($id) : '';
                }else{
                    $idList .= !empty($this->filterInt($id)) ? $this->filterInt($id) .',' : '';
                }
            }
            if(!empty($idList)){
                $this->orders_model::$id = $idList;
                $this->orders_model::$tableSchema['shipping_admin_id'] = $this->filterString($_POST['shipping_admin_id']);
                $this->orders_model::$tableSchema['supervisor_id'] = $this->getSession('user_id');
                $this->orders_model::$tableSchema['order_status'] = SEND_TO_SHIPPING_ADMIN;
                if($this->orders_model->save()){
                    echo 'dn';
                }else{
                    echo 'error';
                }
            }else{
                echo 'error';
            }
        }
    }
}
