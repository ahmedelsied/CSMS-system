...
class order_under_shipping_controller extends abstract_controller
{
    ...
    public function default_action($params)
    {
        $this->get(function ($params)
        {
            if($this->setParams($params,0)){
                $this->active_link();
                $seleced = 'orders.*,government.government as government_name,shipping_admin.full_name as shipping_admin_full_name';
                $this->review_orders = $this->orders_model->getJOIN($seleced,['government','shipping_admin'],['id','id'],['government','shipping_admin_id'],0,'order_status = '.SEND_TO_SHIPPING_ADMIN.' ORDER BY id','LEFT');
                $this->loadHeader(SUPERVISOR_TEMP);
                $this->renderNav(SUPERVISOR_TEMP);
                $this->_view(SUPERVISOR_VIEWS.'shipping_admin_orders_view');
    ...
    public function show_more_action($params)
    {
        $this->ajax(function ($params){
            if($this->setParams($params,0)){
                ...
                $seleced = 'orders.*,government.government as government_name,shipping_admin.full_name as shipping_admin_full_name';
                $this->orders = $this->orders_model->getJOIN($seleced,['government','shipping_admin'],['id','id'],['government','shipping_admin_id'],$offset,'order_status = '.SEND_TO_SHIPPING_ADMIN);
