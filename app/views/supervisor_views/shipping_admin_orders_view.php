            ...
            <div class="col-xs-12">
                ...
                <div class="container-fluid">
                    <div class="pull-right">
                        ...
                        <select class="select-sales select-shipp">
...
<script type="text/javascript">
        ...
        $(document).on('click','#change-shpping-admin',function(){
            var shipping_admin   = Number($('.select-shipp').val()),
                ...
                $(checked).each(function(i, obj) {
                    if($(this).attr('data-shipp') == shipping_admin){
                        console.log('exist');
                    }else{
        ...
        window.handle_message_from_other_pages = function (e){
            var data = JSON.parse(e.data),
                selected = $('.select-shipp').children('option[value="'+data['shipping_admin_id']+'"]').text();
            if(data['status'] == 'updated'){
                for(var id =0; id < data['idList'].length; id++){
                    $("#myTable tr").find("[data-id='" + data['idList'][id] + "']").text(selected);
                }
                var result  = $('.result');
            ...
            }else if(data['status'] == 'recived'){
                for(var id =0; id < data['idList'].length; id++){
                    $("#myTable tr").find("[data-id='" + data['idList'][id] + "']").parents('tr').remove();
                }
            }else if(data['status'] == 'edit_content_under_review' || data['status'] == 'delete_content_under_review'){
